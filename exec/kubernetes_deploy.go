...
func (deployer *KubernetesDeployer) Deploy(name string, filePaths []string, prune bool) error {
	if len(filePaths) == 0 {
...
func (deployer *KubernetesDeployer) Remove(name string, filePaths []string) error {
	if len(filePaths) == 0 {
		return errors.New("missing file paths")
	}
